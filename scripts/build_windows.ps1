...
foreach ($target in $targets) {
    ...
    cargo build --release --target $target.name
}
...
cbindgen --config cbindgen.toml --crate $crate --output dist\include\ddg_suggestions_processor.h
...
Write-Host "   - Header: dist\include\ddg_suggestions_processor.h"
